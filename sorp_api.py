...
api = Api(app, title='Clothing Prediction API', description='Takes clothing images and returns solid or pattern')
...
def require_api_key(f):
    def decorated_function(*args, **kwargs):
        ...
        if api_key is None:
            ...
            abort(403, 'Unauthorized Access: No API Key provided')

        if api_key != 'unicorn':
            log_request(api_key, False)
            abort(403, 'Unauthorized Access: Invalid API Key')
...
def log_request(api_key, is_valid):
    log_entry = APIRequest(api_key=api_key, is_valid=is_valid, timestamp=datetime.now())
...
class Predict(Resource):
    ...
    @require_api_key
    @api.doc(description="Takes a JSON file with a Base64 image and returns predictions")
    def post(self):
